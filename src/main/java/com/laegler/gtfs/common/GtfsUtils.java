...
public class GtfsUtils {
  ...
  public GtfsUtils(PipelineOptions options) {
  ...
  public void setupPubsub() throws IOException {
    GtfsPubsubTopicAndSubscriptionOptions pubsubOptions =
        options.as(GtfsPubsubTopicAndSubscriptionOptions.class);
    if (!pubsubOptions.getPubsubTopic().isEmpty()) {
      ...
      if (!pubsubOptions.getPubsubSubscription().isEmpty()) {
        setupPubsubSubscription(pubsubOptions.getPubsubTopic(),
   ...
   *
   * <p>
   * If the table already exists, the schema has to match the given one. Otherwise, the example will
   * throw a RuntimeException. If the table doesn't exist, a new table with the given schema will be
   * created.
   *
   * @throws IOException if there is a problem setting up the BigQuery table
   */
  public void setupBigQueryTable() throws IOException {
    GtfsBigQueryTableOptions bigQueryTableOptions = options.as(GtfsBigQueryTableOptions.class);
  ...
  private void tearDown() {
    ...
    if (!pubsubOptions.getPubsubTopic().isEmpty()) {
      try {
        ...
        pendingMessages
            .add("Failed to delete the Pub/Sub topic : " + pubsubOptions.getPubsubTopic());
      }
      if (!pubsubOptions.getPubsubSubscription().isEmpty()) {
        try {
    ...
    if (bigQueryTableOptions.getBigQueryDataset() != null
      ...
      pendingMessages.add("The BigQuery table might contain the example's output, "
          + "and it is not deleted automatically: " + bigQueryTableOptions.getProject() + ":"
  ...
  private static Bigquery.Builder newBigQueryClient(BigQueryOptions options) {
    return new Bigquery.Builder(Transport.getTransport(), Transport.getJsonFactory(),
        chainHttpRequestInitializer(options.getGcpCredential(),
  ...
  private static Pubsub.Builder newPubsubClient(PubsubOptions options) {
    return new Pubsub.Builder(Transport.getTransport(), Transport.getJsonFactory(),
        chainHttpRequestInitializer(options.getGcpCredential(),
            ...
            // caller.
            new RetryHttpRequestInitializer(ImmutableList.of(404))))
                .setRootUrl(options.getPubsubRootUrl()).setApplicationName(options.getAppName())
                .setGoogleClientRequestInitializer(options.getGoogleApiTrace());
  }

  private static HttpRequestInitializer chainHttpRequestInitializer(Credentials credential,
      HttpRequestInitializer httpRequestInitializer) {
    if (credential == null) {
      return new ChainingHttpRequestInitializer(new NullCredentialInitializer(),
          httpRequestInitializer);
    } else {
      return new ChainingHttpRequestInitializer(new HttpCredentialsAdapter(credential),
          httpRequestInitializer);
    }
  }
  ...
  private void setupBigQueryTable(String projectId, String datasetId, String tableId,
    ...
    if (table == null) {
      ...
      throw new RuntimeException("Table exists and schemas do not match, expecting: "
          + schema.toPrettyString() + ", actual: " + table.getSchema().toPrettyString());
    }
  }
  ...
  public void waitToFinish(PipelineResult result) {
    ...
    if (!options.as(GtfsOptions.class).getKeepJobsRunning()) {
      ...
      for (PipelineResult pipelineResult : pipelineResults) {
        ...
        for (int retryAttempts = 6; retryAttempts > 0; retryAttempts--) {
          if (pipelineResult.getState().isTerminal()) {
            ...
            System.out
                .println("The example pipeline is still running. Verifying the cancellation.");
          }
